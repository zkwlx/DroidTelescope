...
package andr.perf.monitor.reflect_utils;
...
public class FieldUtils {
    ...
    public static Field getField(Class<?> cls, String fieldName, final boolean forceAccess) {
        ...
        // superclass field.
        Field match = null;
        for (final Class<?> class1 : CommonUtils.getAllInterfaces(cls)) {
            try {
                final Field test = class1.getField(fieldName);
                Validate.isTrue(match == null, "Reference to field %s is ambiguous relative to %s"
                                + "; a matching field exists on two or more implemented " +
                                "interfaces.",
                        fieldName, cls);
                match = test;
            } catch (final NoSuchFieldException ex) { // NOPMD
    ...
    /**
     * Value object's "fieldName" field write to target object.
     *
     * @param target
     * @param fieldName
     * @param srcObject
     * @throws IllegalAccessException
     */
    public static void replaceField(final Object target, final String fieldName, final Object
            srcObject) throws IllegalAccessException {
        Validate.isTrue(target != null && srcObject != null, "target object must not be null");
        final Class<?> cls = srcObject.getClass();
        final Field field = getField(cls, fieldName, true);
        Validate.isTrue(field != null, "Cannot locate declared field %s.%s", cls.getName(),
                fieldName);
        // already forced access above, don't repeat it here:
        replaceField(field, target, srcObject, true);
    }

    public static void replaceField(final Field field, final Object target, final Object srcObject,
                                    final boolean forceAccess)
            throws IllegalAccessException {
        Validate.isTrue(field != null, "The field must not be null");
        if (forceAccess && !field.isAccessible()) {
        ...
        field.set(target, field.get(srcObject));
    }
